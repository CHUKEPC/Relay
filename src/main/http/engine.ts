...
export async function runRequest(
  ...
  // Runs after the body + headers are assembled so the signature covers them.
  if (spec.auth && ['oauth1', 'aws', 'hawk', 'akamai'].includes(spec.auth.type)) {
    const urlencodedParams =
      spec.body?.type === 'urlencoded'
        ? Object.fromEntries(spec.body.items.filter((i) => i.enabled && i.key).map((i) => [i.key, i.value ?? '']))
        : undefined
    const bodyForSign = typeof encoded.body === 'string' || Buffer.isBuffer(encoded.body) ? encoded.body : undefined
    ...
    const unsignedBody = spec.body?.type === 'formdata'
