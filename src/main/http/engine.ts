...
export function buildAuthHeaders(auth: Auth | undefined): {
  ...
  switch (auth.type) {
    ...
    case 'basic': {
      // Like Bearer and API key: an empty form adds nothing rather than `Basic Og==`.
      if (!auth.username && !auth.password) return { headers: {} }
...
/**
 * Headers that are excluded from a request-bound signature's input: they are
 * produced by the signer itself, or rebuilt for every hop, so carrying a stale
 * value into the canonical request would invalidate the signature.
 */
const SIGNER_OWNED_HEADERS = new Set([
  'authorization',
  'cookie',
  'x-amz-date',
...
export async function runRequest(
  ...
  // Token-style auth that needs no request binding (JWT Bearer, ASAP).
  const tokenAuth = buildTokenAuth(spec.auth)
  if (tokenAuth?.error) return errorResult({ kind: 'auth', message: tokenAuth.error }, startedAt, initialUrl, [])
  if (tokenAuth) for (const [k, v] of Object.entries(tokenAuth.headers)) headers[k] = v

  // API key / token placed in the query string.
  const queryAuth = auth.query ?? tokenAuth?.query
  if (queryAuth) {
    try {
      const withKey = new URL(initialUrl)
      withKey.searchParams.append(queryAuth.key, queryAuth.value)
  ...
  // --- 3b. Request-bound auth signing (OAuth1 / AWS SigV4 / Hawk / Akamai). ---
  // The signature covers the method, the URL and the body, so it is recomputed
  // for EVERY hop rather than once: a redirect that changes the path — or that
  // downgrades POST to GET and drops the body — would otherwise replay the
  // signature of the previous request and be rejected by the server.
  const signsRequest = !!spec.auth && ['oauth1', 'aws', 'hawk', 'akamai'].includes(spec.auth.type)
  // Must mirror encodeBody() exactly — the OAuth 1.0 base string has to cover
  // the same parameters, including repeated keys, that go out on the wire.
  const urlencodedParams: Array<[string, string]> | undefined =
    spec.body?.type === 'urlencoded'
      ? (spec.body.items ?? [])
          .filter((i) => i && i.enabled !== false && i.key)
          .map((i) => [i.key, i.value ?? ''] as [string, string])
      : undefined

  /** Sign one hop; returns why signing failed, or null. */
  const applyRequestSignature = (url: string, httpMethod: string): string | null => {
    if (!signsRequest) return null
    const bodyForSign =
      typeof encoded.body === 'string' || Buffer.isBuffer(encoded.body) ? encoded.body : undefined
    // multipart/form-data bytes are generated by undici internally and can't be
    // hashed here → tell AWS SigV4 to use UNSIGNED-PAYLOAD.
    const unsignedBody = encoded.body !== undefined && bodyForSign === undefined
    // Headers the signer owns (or that are rebuilt per hop) must not be fed back
    // into it: Authorization still holds the PREVIOUS hop's signature, which would
    // make the new signature self-referential and unverifiable, and Cookie is
    // reassembled from the jar after signing.
    const signable: Record<string, string> = {}
    for (const [k, v] of Object.entries(headers)) {
      if (!SIGNER_OWNED_HEADERS.has(k.toLowerCase())) signable[k] = v
    }
    const signed = signRequest(spec.auth, {
      method: httpMethod,
      url,
      headers: signable,
      body: bodyForSign,
    ...
    if (signed) for (const [k, v] of Object.entries(signed.headers)) setHeaderCI(headers, k, v)
    return null
  }
  ...
  // Cleared by a cross-origin redirect: from then on no credential — neither a
  // forwarded header nor a freshly computed signature — may leave the process.
  ...
  // A `stale=true` challenge buys exactly one more attempt (never a loop).
  let digestStaleRetried = false
  ...
  let refreshedAuth: ResponseResult['refreshedAuth']
  const withRefreshed = (result: ResponseResult): ResponseResult => (refreshedAuth ? { ...result, refreshedAuth } : result)
  ...
  try {
    ...
    for (let hop = 0; ; hop++) {
      ...
      if (credentialsAllowed) {
        const signError = applyRequestSignature(currentUrl, method)
        if (signError) return errorResult({ kind: 'auth', message: signError }, startedAt, currentUrl, redirects)
      }
      ...
      if (status === 401 && digestAuth) {
        const wwwAuth = firstHeader(res.headers, 'www-authenticate')
        const challenge = wwwAuth ? parseDigestChallenge(wwwAuth) : null
        // RFC 7616 §3.3: `stale=true` means the nonce expired, NOT that the
        // credentials are wrong, so the client may answer once more with the
        // fresh nonce. Without this a preemptive request carrying a saved nonce
        // can never recover, and a normal challenge/response loses the race with
        // a nonce that expires between the two hops.
        const staleRetry = challenge?.stale === true && !digestStaleRetried
        if (challenge && (!digestTried || staleRetry)) {
          await res.body.dump?.()
          if (staleRetry) digestStaleRetried = true
          digestTried = true
      ...
      if (status === 401 && oauth2Auth && !oauth2Refreshed) {
        ...
        if (refreshed.ok && refreshed.accessToken) {
          ...
          refreshedAuth = { accessToken: refreshed.accessToken, ...(refreshed.refreshToken ? { refreshToken: refreshed.refreshToken } : {}) }
      ...
      if (followRedirects && isRedirect && locationStr && hop < maxRedirects) {
        ...
        try {
          ...
          return withRefreshed(await finalizeResponse(res, currentUrl, status, redirects, startedAt, ttfbMs))
        }
        ...
        if (!sameOrigin(currentUrl, nextUrl)) {
          ...
          // Signing side-channel headers carry no secret but describe a signature
          // that no longer exists; drop them with it.
          deleteHeaderCI(headers, 'x-amz-date')
          deleteHeaderCI(headers, 'x-amz-content-sha256')
