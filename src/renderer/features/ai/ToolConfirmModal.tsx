...
export function ToolConfirmModal() {
  ...
  return (
    <Modal open onOpenChange={(o) => !o && confirmTool(false)} width={520} title={undefined}>
      ...
      <div style={{ fontSize: 11.5, color: 'var(--tx-3)', marginTop: 8 }}> {tr('Подтвердите, чтобы ассистент применил изменение. Авто-применение можно включить в Настройках.')} </div>
      <div className="modal-foot">
        <button className="btn" onClick={() => confirmTool(false)}> {tr('Отклонить')} </button>
        <button className="btn primary" onClick={() => confirmTool(true)}>
          <Icon name="check" size={14} /> {tr('Применить')} </button>
      </div>
    </Modal>
  )
}
