...
const TOPICS: { id: TopicId; label: string }[] = [
  ...
  { id: 'isolation', label: 'Сеть и данные' },
  ...
  return (
    <>
      ...
      <Card title={tr('Панели переносятся мышью')}>
        <ul>
          <li>{tr('Боковую панель (коллекции, история, окружения, раннер, консоль, настройки) можно закрепить слева, справа или снизу. Потяните её заголовок «Навигация» — у краёв окна подсветятся места, куда она встанет; отпущенная в любом другом месте, панель останется где была. Плавающей её делает кнопка в заголовке.')}</li>
          <li>{tr('Внутри панели запроса двигаются обе части: ответ — за строку статуса или ручку рядом с кнопками положения, сам запрос — за ручку слева от его названия. Их можно ставить сверху, снизу, слева и справа друг от друга; вторая часть занимает противоположную сторону.')}</li>
          <li>{tr('Навигация и ответ не толкают друг друга: если переставить одну на край, где стоит другая, они меняются местами. Кнопки положения есть в заголовке каждой панели; плавающая панель двигается за заголовок и меняет размер за правый нижний угол.')}</li>
          <li>
...
function IsolationTopic(): JSX.Element {
  return (
    <>
      <div className="help-topic-title">{tr('Сеть и данные')}</div>
      <Card title={tr('Сетевые соединения')}>
        <p>{tr('Соединения открываются при действиях пользователя: отправка запроса, подключение по протоколу, запуск раннера, скрипт с pm.sendRequest, кнопка «Проверить сейчас» в «О приложении», а также запросы к AI-провайдеру и плагинам с разрешением на сеть, если они включены. Проверка обновлений, телеметрия и отчёты о сбоях в фоне не выполняются.')}</p>
      </Card>
      <Card title={tr('Ограничения окна приложения')}>
        <ul>
...
function FaqTopic(): JSX.Element {
  return (
    <>
      ...
      <FaqItem q={tr('Как поделиться парой запросов с коллегой?')}> {tr('Правый клик по запросу или папке в дереве коллекций → экспорт в JSON. Коллега импортирует файл через диалог «Импорт» (формат Postman v2.1) — в Relay или в любом клиенте, который читает этот формат.')} </FaqItem>
      ...
      <FaqItem q={tr('Как включить темы Postman, Insomnia и другие из пака?')}>
      ...
      <FaqItem q={tr('Как переместить боковую панель или панель ответа?')}>
        {tr('Потяните заголовок панели мышью к левому, правому или нижнему краю — она закрепится там; навигация и ответ при этом меняются местами, а не сдвигают друг друга. Плавающей панель делает кнопка в её заголовке. Подробнее — «Справка → Интерфейс и панели».')}
      </FaqItem>
      <FaqItem q={tr('Можно ли просто перетащить файл коллекции в окно?')}>
        {tr('Да. Перетащите из проводника на окно Relay коллекцию Postman или Insomnia, OpenAPI/Swagger, HAR, файл с командой cURL, экспорт окружения Postman, .env или CSV с переменными, тему Relay — каждый файл распознаётся сам. Можно сразу несколько. Резервные копии восстанавливаются только в «Настройки → Данные».')}
      </FaqItem>
      ...
      <FaqItem q={tr('Почему Relay не проверяет обновления сам?')}>
        {tr('Проверка выполняется вручную: кнопка «Проверить сейчас» в «Настройки → О приложении».')}
      </FaqItem>
