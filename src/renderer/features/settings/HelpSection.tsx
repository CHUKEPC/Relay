  ...
  return (
    <>
      ...
      <Card title="pm.* API">
        <ul>
          <li>
            <code>pm.test(name, fn)</code> {tr('и')} <code>pm.expect(...)</code> — проверки;
          </li>
          <li>
    ...
    <>
      ...
      <Card title={tr('Резервная копия SQLite')}>
        <p>
          ...
          один файл <code>.sqlite</code> {tr('и импортируется обратно. Основное хранилище приложения остаётся JSON.')} </p>
      </Card>
    </>
  )
}
  ...
  return (
    <>
      ...
      <Card title={tr('Как подключить')}>
        ...
        <p style={{ marginTop: 8 }}>
          Ключ шифруется системным хранилищем (Keychain / DPAPI / libsecret) и отправляется только
      ...
      <Card title={tr('Какие ИИ можно подключить')}>
        <ul>
          ...
          <li>
            <b>OpenRouter</b> — один ключ (<code>openrouter.ai/keys</code>) даёт доступ к моделям
            Google Gemini, Meta Llama, DeepSeek, Mistral, Qwen, xAI Grok, а также Claude и GPT.
          </li>
          <li>
            <b>Ollama</b> — локальные модели без интернета и ключа. Base URL{' '}
            <code>http://localhost:11434/v1</code>, модель сначала скачайте командой{' '}
            <code>ollama pull llama3.1</code>.
          </li>
          <li>
            <b>LM Studio</b> — локальные модели. Включите сервер во вкладке Developer, Base URL{' '}
            <code>http://localhost:1234/v1</code>.
          </li>
          <li>
            <b>{tr('Другой OpenAI-совместимый')}</b> — любой сервер с эндпоинтом{' '}
            <code>/v1/chat/completions</code>{tr(': Groq, Together, DeepSeek, Mistral, vLLM, LocalAI, корпоративный шлюз. Укажите его Base URL и ключ, если он нужен.')} </li>
        </ul>
      </Card>
      <Card title={tr('Откуда берётся список моделей')}>
        <p>
          После подключения Relay запрашивает у провайдера список доступных моделей (эндпоинт{' '}
          <code>/models</code>) — показываются все модели, доступные вашему ключу. Список
          обновляется кнопкой «Обновить» в выборе модели. Если провайдер не отдаёт список, впишите
          название модели вручную в поле поиска и нажмите «Использовать».
        </p>
      </Card>
      <Card title={tr('Контекст запроса')}>
        <p> {tr('Ассистент открывается по')} <Kbd>{MOD}</Kbd> <Kbd>J</Kbd>. Если в «Настройки → Основные»
    ...
    <>
      ...
      <Card>
        <p>
          ...
          переназначить под себя. <Kbd>{MOD}</Kbd> <Kbd>Z</Kbd> {tr('отменяет последнее изменение в поле запроса, а вне полей — последнее изменение во вкладке.')} </p>
      </Card>
    </>
  )
}
  ...
  return (
    <>
      ...
      <FaqItem q="Как подключить локальную модель (Ollama / LM Studio)?">
        «Настройки → AI-провайдеры» → «Добавить провайдера» → Ollama или LM Studio. Адрес
        подставится сам (<code>http://localhost:11434/v1</code> или{' '}
        <code>http://localhost:1234/v1</code>), ключ не нужен — нажмите «Подключить без ключа».
      </FaqItem>
      ...
      <FaqItem q="Как узнать о новых версиях?"> {tr('В разделе «О приложении» включите проверку обновлений — приложение сверяет свою версию с релизами на GitHub и сообщает, когда вышла новая.')} </FaqItem>
      <FaqItem q="Почему запрос работает здесь, но падает в браузере?"> {tr('Relay отправляет запросы из основного процесса Electron, без браузерных ограничений CORS. Если браузер блокирует запрос, а Relay — нет, проверьте CORS-заголовки на сервере.')} </FaqItem>
      ...
      <FaqItem q="Как сделать резервную копию всех данных?">
        ...
        в один файл <code>.sqlite</code> {tr('и обратный импорт на любом компьютере.')} </FaqItem>
      <FaqItem q="Куда писать с предложениями и багами?"> {tr('На')} <code>{FEEDBACK_EMAIL}</code>{tr('. В разделе «О приложении» есть кнопки «Написать» и «Копировать» для этого адреса.')} </FaqItem>
    </>
  )
}
