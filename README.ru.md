# Relay — изолированный десктопный клиент для тестирования API

*[English version](README.md)*

Кроссплатформенный десктопный **API-клиент** (аналог Postman) для **Windows, macOS и Linux**,
написанный на **Electron + React + TypeScript**.

**Изоляция заложена в устройство приложения.** Всё хранится на вашем компьютере: коллекции, окружения,
история и настройки — это JSON-файлы в вашем пользовательском каталоге, секреты попадают в хранилище ключей
ОС; нет ни учётной записи, ни облака, ни телеметрии, ни фоновых обращений «домой» — приложение не открывает
ни одного соединения, о котором вы не просили. Весь трафик — это отправленные вами запросы и, если вы его
включите, выбранный вами же LLM-провайдер.

**AI-ассистент** доступен как необязательный комплект: подключите любого провайдера (OpenAI,
Anthropic/Claude, OpenRouter или любой OpenAI-совместимый endpoint, включая локальную модель) со своим
API-ключом. По умолчанию он выключен, и приложение полностью работоспособно без него.

> «Relay» — название продукта (задаётся в `package.json` → `productName` и в `src/shared/constants.ts`
> → `APP_NAME`). Чтобы переименовать продукт, измените эти два места.

## Основные возможности

- **Изоляция**: нет учётной записи, облака, телеметрии и автоматической проверки обновлений. Интерфейс
  ничего не загружает из интернета (это обеспечивает строгая CSP), и каждое соединение, которое открывает
  приложение, — это соединение, о котором вы попросили.
...
- **Перемещаемые панели**: боковую панель и панель ответа можно закрепить слева, справа, снизу или сделать
  плавающими — перетаскиванием за заголовок или одним щелчком.
...
- **Необязательный AI-ассистент** (комплект, по умолчанию выключен) с потоковой передачей ответа,
  контекстом с замаскированными секретами и вызовом инструментов с подтверждением.
...
- **Всё локально**: все данные сохраняются в JSON в `userData`; API-ключи шифруются через Electron
  `safeStorage`. Никакой телеметрии; единственный исходящий трафик — ваши собственные API-запросы и (если
  включено) обращения к AI.
